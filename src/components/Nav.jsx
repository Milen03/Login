...
import { href, Link } from 'react-router-dom' // Увери се, че пакетът е react-router-dom
    ...
    <header className="absolute inset-x-0 top-0 z-50">
      <nav aria-label="Global" className="flex items-center justify-between p-6 lg:px-8">
        <div className="flex lg:flex-1">
          {/* Тук можеш да сложиш логото си в Link */}
              ...
              <div className="space-y-2 py-6">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    onClick={() => setMobileMenuOpen(false)} // Затваря менюто при клик
